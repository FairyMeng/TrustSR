...
class Trainer(object):
    ...
    def train(self, start_epoch, end_epoch, start_time=None):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            loss, recall, mrr, accuracy, ndcg = self.evaluation.eval(self.eval_data, self.batch_size)
            print("Epoch: {}, train loss: {:.4f}, loss: {:.4f}, recall: {:.4f}, mrr: {:.4f}, accuracy: {:.4f}, ndcg = {:.4f}, time: {}".format(epoch, train_loss, loss, recall, mrr, accuracy, ndcg, time.time() - st))
            checkpoint = {
                'model': self.model,
                'args': self.args,
                'epoch': epoch,
    ...
    def train_epoch(self, epoch):
        ...
        for ii, (input, target, mask) in tqdm(enumerate(dataloader), total=len(dataloader.dataset.df) // dataloader.batch_size, miniters = 1000):
            ...
            evidences, evidence_a, loss = self.model(input, hidden, target, epoch)
            # output sampling
